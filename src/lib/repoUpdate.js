...
export async function LoadJSON(client) {
  ...
  client.user.setPresence({
    activities: [
      {
        name: `${client.packageCount.toLocaleString()} packages`,
...
export async function RepoUpdater(repoName, repoURL) {
  ...
  try {
    ...
    console.log(
      "[ ERROR ] : Data directory didn't exist. Logged to errors.txt"
    );
    logError(e);
  }
  fs.mkdirSync("./data");
  try {
    ...
    await streamPipeline(
      response.body,
      fs.createWriteStream("./data/repo.pkgs")
    );
    skip = true;
  } catch (e) {
  ...
  if (skip == false) {
    try {
      ...
      await streamPipeline(
        response.body,
        fs.createWriteStream("./data/Packages.bz2")
      );
    } catch (e) {
      console.log(
        `[ ERROR ]: Repo ${repoName} had an error downloading the BZ2. Logged to errors.txt`
      );
      logError(e);
      return false;
    }
