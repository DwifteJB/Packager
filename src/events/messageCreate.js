...
module.exports = async (client, message) => {
  ...
  if (blacklist.includes(message.author.id))
    return message.channel.send("Yeah, you're uh. blacklisted.");
  const now = Date.now();
  const expiration = client.cooldowns.get(message.author.id);
  if (expiration) {
    if (expiration > now)
      return message
        .reply(`Please wait ${ms(expiration - now)} before searching again.`, {
  ...
  if (shitTweaks.includes(package))
    return message.channel.send(
      "I would rather you not try to break your device."
    );
  let sent = false;
  ...
  try {
    client.jsons.forEach((repo) => {
      ...
      for (index in repo.app) {
        ...
        if (
          (Data.Name ? Data.Name.toLowerCase() : "").startsWith(package) ||
          package === (Data.Package ? Data.Package.toLowerCase() : "")
        ) {
          Data.Icon = !Data.Icon
            ? "https://upload.wikimedia.org/wikipedia/commons/f/fb/Icon_Sileo.png"
            : Data.Icon;
          ...
          const lmao = new EmbedBuilder()
            ...
            .setAuthor({
              name: Data.Name ? Data.Name.trim() : Data.Package.trim(),
            });
    ...
    console.log(err);
    return message
      .reply("Error occured, report was sent to Dwifte.", {
        allowedMentions: { repliedUser: false },
      })
      .then((msg) => {
  ...
  if (!sent) {
    return message
      .reply("I couldn't find anything matching that search query!", {
        allowedMentions: { repliedUser: false },
      })
      .then((msg) => {
        setTimeout(() => msg.delete(), 5000);
  ...
  collector.on("collect", (interaction) => {
    row.components[0].setPlaceholder(
      Embeds[interaction.values[0]].data.author.name || "Not Found!"
    );
    EmbedMSG.edit({
      components: [row],
