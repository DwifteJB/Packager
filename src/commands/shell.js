...
module.exports = {
  Data: new SlashCommandBuilder()
    ...
    .addStringOption((option) =>
      option.setRequired(true).setName("code").setDescription("Code to run")
    ),
  async execute(client, message) {
    ...
    exec(Command, (error, output, stderr) => {
      ...
      if (output.length < 1) return msg.edit("Finished with no output.");
      if (output.length > 1994) {
        haste(output).then((haste) => msg.edit("Output was too big: " + haste));
